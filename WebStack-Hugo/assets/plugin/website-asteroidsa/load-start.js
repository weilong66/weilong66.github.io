...
if (link) {
  ...
  link.addEventListener("click", function (event) {
    ...
    if (localStorageValue && localStorageValue === 'true') {
       ...
       *（在线版其实没网也能玩，但是不会显示得分板，也无法正常更换飞船皮肤）
       */
      var img = new Image();
      ...
      img.onerror = function () {
        ...
        console.log("kickassApp站点似乎无法正常访问，WebsiteAsteroids插件改为使用本地版本.");
        localStorage.setItem(localStorageName, 'false'); //如果加载在线版失败则设置为true，避免下次启用插件时仍然重复加载增加加载延迟
      };
    } else {
      loadScript(kickassBaseUrl + "kickass-local.js");
    }
