/*
    ...
    进行了一些本地化修改，包括：文本改为中文；将用到的资源下载到本地，保证断网也能正常运行。基本每个修改过或者读懂的地方都添加了中文注释方便二次修改。
*/


(function (window) {

    let shipIdLocal = 325; //飞船本地皮肤id
    let randomShipLocal = true; //开启随机飞船皮肤
    ...
    var KickAss = new Class({
        ...
        registerElement: function (el) {
            if (!el) {
                throw new Error("Can't register unexisting element(无法注册不存在的元素).");
            }
        ...
        updateShips: function (ship, isInitial) {
            if (!isInitial) {
                this.ui.showMessage("你现在正在驾驶：<br /><em>" + ship.name + "<em>!!");
            }
            for (var i = 0, player; player = this.players[i]; i++) {
                player.setShip(ship);
            }
        ...
        changeWeapon: function (weapon, isInitial) {
            ...
            if (!isInitial) {
                this.ui.showMessage("已更改为 " + weapon.name.toUpperCase() + "!!!!");
            }
            for (var i = 0, player; player = this.players[i]; i++) {
        ...
        newRank: function (rank) {
            this.ui.showMessage("天哪！你升级到: <strong>" + rank + '</strong> 级!<br />');
        },
        fireBomb: function () {
    ...
    var Menu = new Class({
        ...
        /*
        //生成菜单页面（这个是KickAss菜单页面，必须联网才有用，可以上传分数或切换飞船皮肤，但本地版其实用不上的。
        //不过已经把调用此函数的代码注释了，所以这个函数就没注释）
        */
        generate: function (parent) {
            this.container = document.createElement('div');
            this.container.className = 'KICKASSELEMENT';
            this.container.id = 'kickass-profile-menu';
            parent.appendChild(this.container); //将详细菜单作为子菜单添加到得分板菜单
            var shipId = getGlobalNamespace().KICKASSSHIPID || "";
            /* 菜单html的网址，需要在线使用（数据太多没法移植到本地） */
        ...
        messageTypeChangeShip: function (pieces) {
            ...
            this.shipId = shipId;
            //飞船皮肤的json文件地址（单独分离出来方便修改）
            // let constructionUrl = GameGlobals.path('designer/ship/' + shipId + '/construction.json');
            //本地飞船json文件地址
            let constructionUrl = GameGlobalsLocal.path('assets/plugin/website-asteroidsa/kickass/designer/ship/' + shipId + '/construction.json');
            CORS.request(constructionUrl, {
                ship_id: shipId,
        ...
        //删除“菜单页面”元素。这里需要注释掉一些函数，因为没生成菜单页面时调用对应移除函数会出错。
        destroy: function () {
    ...
    var MenuManager = new Class({
        initialize: function (game) {
            ...
            if (!getGlobalNamespace().KICKASS_INLINE_CSS) {
                //使用本地css文件
                let menustylesUri = 'assets/plugin/website-asteroidsa/kickass/css/menustyles.css';
                this.includeCSS(GameGlobalsLocal.path(menustylesUri));
        ...
        create: function () {
            ...
            this.container.className = 'KICKASSELEMENT' + (this.game.shouldShowMenu() ? "" : "KICKASSNOMENU");
            ...
            this.container.innerHTML =
                ...
                (this.game.shouldShowMenu() ? "" : 'style="display:none"') +
                ">" +
            ...
            this.bombLink = document.getElementById('kickass-bomb-menu'); //炸弹菜单
            ...
            if (this.game.shouldShowMenu()) {
                ...
                /*
                //此函数用于生成菜单页面，注释掉就不会生成了
                //（菜单页面是一个单独页面，也是KickAss的，但是因为需要加载在线数据，如飞船模型、账号等级、上传分数排行等，无法下载到本地，并且本地离线版也用不上这些功能）
            ...
            //为“更换飞船”按钮绑定点击事件，因为本地版去掉了菜单，所以这个按钮改成把初始飞船升级
            let switchShipClickIndex = 0; //用来判断是否更换飞船按钮是否点击过
            // 飞船皮肤id数组
            const shipIds = [39, 146, 162, 230, 325, 2712, 9741, 21632, 22988];
            // 飞船皮肤索引
            var shipIdIndex = 0;

            /** 这部分增加了个根据得分判断是否允许升级，但后来觉得没必要就注释掉了，你想用的话取消注释就能直接用了 */
            // let shipUpgradePoints = 0; //飞船升级需要的分数
            addEvent(this.switchShipLink, 'click', bind(this, function (e) {

                //如果没有点击过，就更换飞船，否则就还原为初始飞船
                if (!switchShipClickIndex) {
                    // let newScore = this.points.innerHTML - shipUpgradePoints; //当前得分减去上次升级飞船所需分数
                    // if (newScore >= 500) { //如果得分大于等于500分，就可以升级飞船
                    //     shipUpgradePoints = this.points.innerHTML;//重置为当前得分

                    switchShipClickIndex = 1;
                    //如果文件顶部的randomShipLocal变量值为true，则随机选择一个飞船皮肤
                    if (randomShipLocal) {
                    ...
                    //单独调用“选择飞船”的函数来改变飞船皮肤（也想过直接修改初始化时的飞船外观数据，不过感觉还是这样更方便）
                    // 这里飞船id独立出来放到文件顶部，方便快速修改。（懒得再加个切换菜单来可视化选择不同飞船皮肤，反正好看的也不多）
                    this.menu.messageTypeChangeShip(shipIdLocal + ",1,initial");
                    console.log(this.points.innerHTML);

                    //     } else {
                    //         let scoreDiff = 500 - newScore;
                    //         this.game.ui.showMessage("分数不足，你还差" + scoreDiff + "分才能再次升级飞船！");
                    //     }

                } else {
                    switchShipClickIndex = 0;
                    this.menu.messageTypeChangeShip("ShipsStandard,1,initial");
                }


            }));
            //切换上一个飞船
            addEvent(this.switchPreviousShipLink, 'click', bind(this, function (e) {
                //如果点击过更换飞船按钮，说明已启用飞船皮肤，才能切换其它飞船
                if (switchShipClickIndex) {

                    //如果已经是第一艘飞船，则切换为最后一艘飞船
                    if (shipIdIndex === 0) {
                        shipIdIndex = shipIds.length - 1;
                    } else {
                        shipIdIndex = shipIdIndex - 1;
                    }
                    //使用索引获取飞船id
                    shipIdLocal = shipIds[shipIdIndex];
            ...
            //切换下一个飞船
            addEvent(this.switchNextShipLink, 'click', bind(this, function (e) {
                //如果点击过更换飞船按钮，说明已启用飞船皮肤，才能切换其它飞船
                if (switchShipClickIndex) {

                    //如果已经是最后一搜飞船，则切换为第一艘飞船
                    if (shipIdIndex === shipIds.length - 1) {
                        shipIdIndex = 0;
                    } else {
                        shipIdIndex = shipIdIndex + 1;
                    }
        ...
        getShareHTML: function () {
            /* //这部分是原来的代码，用于为KickAss官方Facebook的公共主页点赞，国内用不了所以进行了修改
            ...
            /* //这里改成了微博的分享链接（其实没什么意义，点击后就只是唤出微博分享窗口，并自动填入 'url' 的值）
            if (typeof getGlobalNamespace().KICKASS_SHARE_URL !== "undefined") {
                if (getGlobalNamespace().KICKASS_SHARE_URL) {
                    var url = encodeURIComponent(
                        getGlobalNamespace().KICKASS_SHARE_URL
                    );
                    return (
                        '<iframe class="KICKASSELEMENT kickass-like" src="https://open.weibo.com/kit/widget/share.html?url=' +
                        url +
                        '&amp;type=button&amp;language=zh_cn&amp;searchPic=true&amp;style=simple" width="65" height="25" frameborder="0" scrolling="no" marginheight="0"></iframe>'
                    );
                } else {
                    return "";
                }
            } else {
                var url = "一起来摧毁烦人的页面：https://kickassapp.com/";
                if (this.game.mySite) {
                    if (this.game.mySite.getShareURL()) {
                        url = this.game.mySite.getShareURL();
                    } else {
            ...
            return (
                ...
                url +
                '&amp;language=zh_cn;" style="width: 108px;top: 28px;" width="108" height="25" frameborder="0" scrolling="no" marginheight="0"></iframe>'
            );
        },
        onGameReady: function () {
    ...
    var BombManager = new Class({
        ...
        update: function (tdelta) {
            ...
            if (this.nextBomb < 0) {
                ...
                this.nextBomb = -1;
                this.game.ui.showMessage("炸弹准备好了<br />(点击右下角炸弹图标或按下 F )");
            }
        },
        blow: function () {
    ...
    var SessionManager = new Class({
        ...
        weHaveWon: function () {
            this.isPlaying = false;
            this.game.ui.showMessage("你已摧毁页面!");
    ...
    var LaserBullet = new Class({
        ...
        initCanvas: function () {
            ...
            var s = Math.max(GameGlobalsLocal.laserImage.width, GameGlobalsLocal.laserImage.height);
    ...
    GameGlobalsLocal.laserImage = document.createElement('img');

    //使用本地laser.png图片
    // GameGlobals.laserImage.src = GameGlobals.path('css/gfx/kickass/laser.png');
    GameGlobalsLocal.laserImage.src = GameGlobalsLocal.path('assets/plugin/website-asteroidsa/kickass/css/laser.png');
    ...
    var Sheet = new Class({
        ...
        drawLaser: function (pos, dir) {
            ...
            this.drawer.drawImage(GameGlobalsLocal.laserImage, 0, 0);
        },
