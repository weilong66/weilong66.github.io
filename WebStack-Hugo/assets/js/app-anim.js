(function ($) {
    ...
    //该函数作用是点击页面上所有非空白目标的链接时，检查一些条件，并在满足这些条件的情况下显示一个加载动画
    /* $(document).on('click', "a[target!='_blank']", function() {
        // 检查主题加载设置是否为 '1'，并且链接存在且不是锚点也不是JavaScript链接，并且没有使用fancybox、commentid属性以及没有nofx类
        if (theme.loading == '1' &&
            $(this).attr('href') &&
            ...
            // 创建一个用于显示加载动画的div元素，并设置其ID为'load-loading'
            var load = $('<div id="load-loading"></div>');

            // 将创建的div元素插入到body标签的最前面
            $("body").prepend(load);

            // 对新创建的div元素执行动画：透明度从0变为1，持续时间为200毫秒，使用swing缓动效果
            // 动画完成后延迟2000毫秒（即2秒），然后将透明度从1变为0，持续时间为300毫秒，最后移除该div元素
    ...
    //这段代码的主要功能是初始化模态框内的表单字段，并在显示模态框时根据按钮的属性值设置相应的输入框内容
