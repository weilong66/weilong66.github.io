...
/**
 * 使用当前a标签中的指定前缀到第一个斜杠(`/`)后的字符串替换当前域名中的子域名作为新域名，替换a标签中的href中的值
 * 例如当前域名为https://local.demo123.com，a标签为<a href="javascript-http//demo">，则将该a标签href替换为<a href="https://test12.demo123.com">
...
function replaceAHref(prefix) {
    ...
    $hostPrefix.each(function () {
        ...
        var match = href.substring(prefix.length).match(/([^\/]*)\/(.*)/);
        if (match && match[1]) {
            // 提取新的子域名
            ...
            if (match[2]) {
                // 构造新的URL（使用协议 + 新构造的URL + 原URI）
                var newUrl = window.location.protocol + "//" + newHostname + '/' + match[2];
            }else{
