    ...
    const bodyWidthMin = 0;
    const bodyWidthMax = 0;
    const bodyHeightMin = 0;
    const bodyHeightMax = 0;


    /*
     创建飞机类
     */
    //参数按顺序：血量、x位置、y位置、x大小、y大小、分数、死亡时间、速度、飞机爆炸图片、飞机图片
    ...
    function ourplan(X, Y) {
        var imagesrc = "image/player.gif";
        plan.call(this, 1, X, Y, 66, 80, 0, 660, 0, "image/player_boom.gif", imagesrc);
    ...
    /*
     创建本方飞机
     */
    var selfplan = new ourplan(120, 485);//创建本方飞机类对象
    var ourPlan = document.getElementById('ourplan'); //获取飞机元素控件
    //本方飞机移动事件函数
    var yidong = function () {
        var oevent = window.event || arguments[0];
        var chufa=oevent.srcElement||oevent.target;
        //获取鼠标的坐标
        var selfplanX = oevent.clientX-400;
        var selfplanY = oevent.clientY;
        console.log("鼠标位置x：",oevent.clientX)
    ...
    var bianjie = function () {
        ...
        if (bodyobjX < 405 || bodyobjX > 715 || bodyobjY < 0 || bodyobjY > 568) {
            if (document.removeEventListener) {
                mainDiv.removeEventListener("mousemove", yidong, true);
            } else if (document.detachEvent) {
                mainDiv.detachEvent("onmousemove", yidong);
    ...
    function start() {
        ...
        if (mark == 20) {
            ...
            //中飞机
            if (mark1 % 5 == 0) {
                enemys.push(new enemy(6, 25, 274, 46, 60, 5000, 360, random(1, 3), "image/medium_boom.gif", "image/enemy3_fly_1.png"));
            }
            //大飞机
            if (mark1 == 20) {
                enemys.push(new enemy(12, 57, 210, 110, 164, 30000, 540, 1, "image/big_boom.gif", "image/enemy2_fly_1.png"));
                mark1 = 0;
            }
            //小飞机
            else {
                enemys.push(new enemy(1, 19, 286, 34, 24, 1000, 360, random(1, 4), "image/small_boom.gif", "image/enemy1_fly_1.png"));
            }
            mark = 0;
        }
                ...
                if (enemys[j].planisdie == false) {
                    if (enemys[j].imagenode.offsetLeft + enemys[j].plansizeX >= selfplan.imagenode.offsetLeft && enemys[j].imagenode.offsetLeft <= selfplan.imagenode.offsetLeft + selfplan.plansizeX) {
                        if (enemys[j].imagenode.offsetTop + enemys[j].plansizeY >= selfplan.imagenode.offsetTop + 40 && enemys[j].imagenode.offsetTop <= selfplan.imagenode.offsetTop - 20 + selfplan.plansizeY) {
                            //碰撞本方飞机，游戏结束，统计分数
                            selfplan.imagenode.src = "image/player_boom.gif";
