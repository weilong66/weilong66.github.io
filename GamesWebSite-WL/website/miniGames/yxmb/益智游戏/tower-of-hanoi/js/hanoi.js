var delay = 200; //in milliseconds
    ...
    function initializeDrag(disk,e){
    	if (!e) e=event;
        if (stop){
            swal("警告", "自动解答过程中点击“停止”按钮后无法继续解答谜题！\n请单击“重新启动”按钮或选择圆盘数量以重新开始游戏。", "warning");
    ...
    function dropDisk(disk){
        ...
        if (gameStatus) {
            ...
            if (movectr==minmove) msg="\n恭喜! 你只用了 "+movectr+" 步就解决了该谜题。"
            else if (movectr>minmove) msg="\n你用了 "+movectr+" 步来解决该谜题，也许你可以做得更好。"
            else msg="";
            //alert("Game Over !!!"+msg);
    ...
    function solve(btn){
        if (btn.value=="解答"){
            if (movectr>0 && !gameOver && !stop)
                if (!confirm("当前游戏将结束，您确定要继续吗？")) return;
            btn.value="停止";
            initVars();
            stop=false;
            demo=true;
    ...
    function moveDisk(){
        ...
        if (movectr<parseInt(frm.minmove.value)) t=window.setTimeout("moveDisk()",delay);
            ...
            swal({   title: "搞定！",   text: "总共用了 "+movectr+" 步！你还能用更少的步数完成它吗？",   imageUrl: "images/pinpin.png" });
            //swal("Can you do that in "+movectr+" moves?");
            gameOver=true;
            stop=false;
            frm.btnSolve.value="解答";
            frm.btnIns.disabled=false;
            frm.btnRes.disabled=false;
        }
    }
    ...
    function displayIns(){
        var msg = "你要将所有的圆盘从TOWER 1移动到TOWER 3\n";
        msg += "你每次只能移动一个圆盘。\n";
        msg += "游戏最重要的规则是大的圆盘不能放在小的圆盘上面！";
