    ...
    for child in sorted(os.listdir(path)):  # 获取并排序所有子项
        ...
        if os.path.isdir(full_path):  # 只处理子目录
            ...
            if depth < 1:  # 最多识别每个目录的子目录
                ...
                metadata = read_metadata_file(full_path)  # 读取metadata.json文件
                game_name = os.path.basename(full_path)
                game_alias = metadata.get('name', os.path.basename(full_path))
                img_path = metadata.get('img_path', find_image_path(full_path, game_name))
                desc_text = metadata.get('desc', read_desc_file(full_path))  # 读取描述文件
                main_page = metadata.get('main_page', find_main_page(full_path))  # 查找主页文件
                d['children'].append({'name': game_name,'game_alias':game_alias, 'type': 'games', 'desc': desc_text, 'img_path': img_path, 'main_page': main_page})
...
def find_image_path(game_dir, game_name):
    ...
    return f'../yxmb/{os.path.basename(os.path.dirname(game_dir))}/{game_name}'  # 默认图片路径
...
def find_main_page(game_dir):
    index_files = ['index.html', 'index.htm']
    other_html_files = [f for f in os.listdir(game_dir) if f.endswith(('.html', '.htm')) and f not in index_files]
    
    for index_file in index_files:
        index_file_path = os.path.join(game_dir, index_file)
        if os.path.exists(index_file_path):
            return index_file
    
    if other_html_files:
        return other_html_files[0]
    
    return None

def read_metadata_file(game_dir):
    metadata_file_path = os.path.join(game_dir, 'metadata.json')
    if os.path.exists(metadata_file_path):
        with open(metadata_file_path, 'r', encoding='utf-8') as file:
            try:
                return json.load(file)
            except json.JSONDecodeError:
                print(f"Failed to decode JSON from {metadata_file_path}")
                return {}
    return {}
    ...
    """
        ...
        for game in category.get('children', []):
            if game['type'] == 'games':
                ...
                href = f"../yxmb/{category['name']}/{game['name']}/{game['main_page']}" if game['main_page'] else "#"
                img_text = game['game_alias']
                desc_text = game['desc']
                img_path = game.get('img_path', '../default.jpg')
                if not img_path.startswith('../yxmb'):
                    img_path = f"../yxmb/{category['name']}/{game['name']}/{img_path}"
...
<body>
    ...
    for category_name, game in games:
        href = f"../yxmb/{category_name}/{game['name']}/{game['main_page']}" if game['main_page'] else "#"
        img_text = game['game_alias']
        desc_text = game['desc']
        img_path = game.get('img_path', '../default.jpg')  # 默认图片路径
        if not img_path.startswith('../yxmb'):
                    img_path = f"../yxmb/{category_name}/{game['name']}/{img_path}"
